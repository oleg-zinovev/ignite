...
import org.apache.ignite.internal.processors.query.calcite.exec.exp.window.FramingFactory;
import org.apache.ignite.internal.processors.query.calcite.exec.exp.window.WindowAccumulatorsFactory;
...
public class ExpressionFactoryImpl<Row> implements ExpressionFactory<Row> {
    ...
    /** {@inheritDoc} */
    @Override public IntFunction<Framing<Row>> windowFrameFactory(
        Window.Group group,
        List<AggregateCall> calls,
        RelDataType rowType
    ) {
        ...
        WindowAccumulatorsFactory<Row> accFactory = new WindowAccumulatorsFactory<>(ctx, calls, rowType);
        return new FramingFactory<>(ctx, group, accFactory, rowType);
    }
