 ...
 */
@Value.Enclosing
public class IgniteWindowConstantsProjectRule extends RelRule<IgniteWindowConstantsProjectRule.Config> implements TransformationRule {
    /**  */
    public static final IgniteWindowConstantsProjectRule INSTANCE = new IgniteWindowConstantsProjectRule(IgniteWindowConstantsProjectRule.Config.DEFAULT);

    private IgniteWindowConstantsProjectRule(Config cfg) {
        super(cfg);
    }
    ...
    private RelNode buildWindowWithoutConstants(LogicalWindow window, RelNode input, RelNode newInput) {
        ...
        // agg calls in window allready reference fields by index,
        // do not need to remap it
        return LogicalWindow.create(window.getTraitSet(), newInput, ImmutableList.of(), type, window.groups);
    }
    ...
    public interface Config extends RelRule.Config {
        /**  */
        IgniteWindowConstantsProjectRule.Config DEFAULT = ImmutableWindowConstantsProjectRule.Config.of()
            .withOperandSupplier(b -> b.operand(LogicalWindow.class)
                .predicate(it -> !it.constants.isEmpty())
                .anyInputs());

        /** {@inheritDoc} */
        @Override default IgniteWindowConstantsProjectRule toRule() {
            return INSTANCE;
        }
    }
}
